...
def calculate_totals_kelly(abs_edge):
    """Calculates Kelly Unit size for TOTALS with higher selectivity."""
    STD_DEV = 13.5
    PAYOUT_RATIO = 0.9091
    MIN_EDGE = 3.0 # Relaxed from 4.6
    MAX_UNITS = 3.0
    KELLY_FRACTION = 0.05
...
def calculate_moneyline_kelly(model_prob, vegas_prob, vegas_odds):
    ...
    units = round(min(max(0, kelly_fraction * 0.10), MAX_UNITS), 1)
    
    if units > 0:
        conf = "SOLID" if edge >= 0.12 else "LEAN"
    else:
        conf = "None"
...
def run_simulation():
    print("🚀 Starting Protocol 705 V3 Simulation...")
    ...
    if not df.empty:
        # df.to_csv(HISTORY_PATH, index=False) # Don't overwrite actual history
        num_spread = len(df[df['type']=='spread'])
        num_total = len(df[df['type']=='total'])
        num_ml = len(df[df['type']=='moneyline'])
        
        profit_spread = df[df['type']=='spread']['profit'].sum()
        profit_total = df[df['type']=='total']['profit'].sum()
        profit_ml = df[df['type']=='moneyline']['profit'].sum()

        print("\n\n=== BALANCED SIMULATION RESULTS ===")
        print(f"SPREAD: {num_spread} bets | {profit_spread:.2f}u Profit")
        print(f"TOTAL:  {num_total} bets | {profit_total:.2f}u Profit")
        print(f"ML:     {num_ml} bets | {profit_ml:.2f}u Profit")
        print(f"TOTAL:  {len(df)} bets | {df['profit'].sum():.2f}u")
    else:
