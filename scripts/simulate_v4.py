...
BACKUP_PATH = "data/betting_history_backup_v3.csv"
...
def calculate_moneyline_kelly(model_prob, vegas_prob, vegas_odds):
    ...
    # V4: Increased Kelly fraction to 0.5 (was 0.12)
    units = round(min(max(0, kelly_fraction * 0.5), MAX_UNITS), 1)
...
def run_simulation():
    print("Starting Protocol 705 V4.0 (Quasar) Simulation...")
    ...
    # v4 Spread Ensemble
    print(f"   Loading v4 Ensemble: {MODEL_STACK_PATH}")
    with open(MODEL_STACK_PATH, 'rb') as f:
        spread_model = pickle.load(f)
        
    with open("models/v4_features.pkl", 'rb') as f:
        v4_features = pickle.load(f)
    
    # v4 Total Ensemble
    print(f"   Loading v4 Total Ensemble...")
    with open("models/v4_total_stack.pkl", 'rb') as f:
        total_model = pickle.load(f)
    with open("models/v4_total_features.pkl", 'rb') as f:
        v4_total_features = pickle.load(f)
    
    # v4 Moneyline Ensemble
    print(f"   Loading v4 Moneyline Ensemble...")
    with open("models/v4_moneyline_stack.pkl", 'rb') as f:
        ml_model = pickle.load(f)
    with open("models/v4_moneyline_features.pkl", 'rb') as f:
    ...
    for week in weeks:
        ...
        for _, game in week_games.iterrows():
            ...
            # 2. TOTAL (Keep existing model/logic for now)
            ...
            if not pd.isna(raw_total):
                ...
                units_total, _ = calculate_totals_kelly(abs_total_edge)
    ...
    if os.path.exists(HISTORY_PATH):
        try:
            ...
            print(f"Could not backup history: {e}")
    ...
    if not df.empty:
        ...
        print("No bets made in simulation.")
